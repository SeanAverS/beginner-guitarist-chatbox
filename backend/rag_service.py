...
# This will answer the user query using a vector index and Gemini as context 
...
def get_retriever():
    ...
    if _retriever is None:
        log("[Retriever] Loading Vector Index...")
...
# search vector index for similar content to user query
def search_vector_db(user_query, k=3):
    retriever = get_retriever() # lazy-loaded retriever
    start = time.time()
    results = retriever.query(user_query, k)
    elapsed = (time.time() - start) * 1000  # milliseconds
    log(f"[VectorDB] Retrieval took {elapsed:.2f} ms")
    return results
...
# construct answer to user query with vector index and Gemini
def rag_answer(user_query):
    # knowledge_docs = search_hnswlib(user_query, k=3) 
    knowledge_docs = search_vector_db(user_query, k=3)

    saved_chats, total_chats, remaining_chats = get_saved_chat_summary()

    # Build context using vector index 
    context_sections = []
    if saved_chats:
    ...
    start = time.time()
    # Construct answer using Gemini and prompt (vector index)
    try:
...
def handle_query(query, chat_filename=None):
    ...
    try:
        ...
        # concise format of vector index
        simplified_docs = []
